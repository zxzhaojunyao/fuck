...
export async function runShell(
  ...
  const timer = setTimeout(() => {
    ...
    // killTree is fire-and-forget; a stubborn/detached child (openvpn, a server,
    // a daemonized proc) may survive the taskkill and leave 'exit' un-fired
    // forever. Force-settle shortly after so the tool ALWAYS returns (as TIMEOUT)
    // instead of hanging the whole agent.
    setTimeout(() => settleExit(null), 2000)
  }, timeout)
  ...
  const exitCode: number | null = await exitCodePromise
