...
` + SUMMARY_TEMPLATE,
  ...
  // load extension modules async (disk discovery). The built-in todo extension is
  // disabled: its per-turn `[todo reminder]` context hook re-anchors the agent onto a
  // possibly-wrong path every turn (e.g. "SSH bruteforce" written as a todo keeps pulling
  // the agent back even after the path is dead, blowing up the context). Re-enable with:
